...
function draw() {
  ...
  //no diamond/stone/spike overlap
  for (var a = 0; a<(stonegroup).length;a++) {
    ...
    for (var b = 0 ; b<(diagroup).length; b++) {
      ...
      for (var c = 0 ; c<(spikegroup).length;c++){
        var tempC = spikegroup.get(c);

     
      if (tempA.isTouching(tempC)) {
        tempC.destroy();
        tempC=null;
      }

      if (tempB.isTouching(tempC)) {
        tempC.destroy();
        tempC=null;
      }
    }
  ...
  /*for (var a = 0; a<(diagroup).length;a++) {
    var tempA = diagroup.get(a);
    var tempB = stonegroup.get(a);
    var tempC = spikegroup.get(a);
